...
/** Ablageort der Kleidungsdaten. Im Add-on das persistente Volume /data. */
export const DATA_FILE = process.env.DATA_FILE
  ? path.resolve(process.env.DATA_FILE)
  : path.resolve(__dirname, '..', '..', 'data', 'einsatzkleidung.json');
...
export const APP_TITEL = text(optionen.titel) || process.env.APP_TITEL?.trim() || 'Einsatzkleidung';
export const APP_UNTERTITEL = text(optionen.untertitel)
  ?? (process.env.APP_UNTERTITEL?.trim() || 'Persönliche Schutzausrüstung, Wäsche & Prüfung');
