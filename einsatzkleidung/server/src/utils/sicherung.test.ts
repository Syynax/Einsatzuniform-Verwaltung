...
const mitDaten = async (inhalt = '{"sorten":[{"id":1}]}'): Promise<string> => {
  const ordner = await fs.mkdtemp(path.join(os.tmpdir(), 'einsatzkleidung-sicherung-'));
  const datei = path.join(ordner, 'einsatzkleidung.json');
...
test('Sicherung legt eine Kopie mit dem Datum im Namen an', async () => {
  ...
  assert.equal(path.basename(pfad), `einsatzkleidung-${heute()}.json`);
...
test('Ohne Datendatei gibt es nichts zu sichern', async () => {
  const ordner = await fs.mkdtemp(path.join(os.tmpdir(), 'einsatzkleidung-leer-'));
...
test('Rotation behält die jüngsten Sicherungen', async () => {
  ...
  for (const tag of ['2026-01-01', '2026-01-02', '2026-01-03', '2026-01-04']) {
    await fs.writeFile(path.join(ordner, `einsatzkleidung-${tag}.json`), '{}', 'utf-8');
  }
  ...
  assert.ok(uebrig.includes(`einsatzkleidung-${heute()}.json`), 'die neue muss bleiben');
  assert.ok(!uebrig.includes('einsatzkleidung-2026-01-01.json'), 'die älteste muss weichen');
});
...
test('Rotation fasst fremde Dateien im Ordner nicht an', async () => {
  ...
  await fs.writeFile(path.join(ordner, 'einsatzkleidung-2026-01-01.json'), '{}', 'utf-8');

  await sichereTaeglich(datei, 1);

  const uebrig = await dateienIm(ordner);
  assert.ok(uebrig.includes('notiz.txt'), 'fremde Dateien bleiben unberührt');
  assert.ok(uebrig.includes(`einsatzkleidung-${heute()}.json`));
});
...
test('Import-Sicherungen werden auf die gewünschte Zahl gestutzt', async () => {
  ...
  assert.deepEqual(uebrig, ['einsatzkleidung.json.backup-2026-03-01T10-00-00-000Z']);
});
